...
for k = 1:10
    ...
    hd =[10 10];
    net1 = feedforwardnet(hd,'trainscg'); 
    net1.trainParam.epochs = 190;
    x=dataTrain';
    t=labelsTrain';
    net1 = train(net1,x,t);
   ...
   %%Predicting the Training Class
    predictTrain=net1(x);
    predictTrain= round(predictTrain);
      
    
    %% Getting the class performance for the training data set
    cp = classperf(t,'Positive',1,'Negative',0);
    cp= classperf(cp,predictTrain);
